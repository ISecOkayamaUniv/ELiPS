...
int efp_compute_y_from_x(fp_t *y, fp_t *x) {
  fp_t tmp1;
  fp_init(&tmp1);
  fp_mul(&tmp1, x, x);
  fp_mul(&tmp1, &tmp1, x);
  fp_add_mpn(&tmp1, &tmp1, curve_b);
  if (fp_legendre_sqrt(y, &tmp1) == 1)
    return 1;
  else
    return 0;
}

int efp_compute_y_from_x_montgomery(fp_t *y, fp_t *x) {
  fp_t tmp1;
  fp_init(&tmp1);
  fp_mulmod_montgomery(&tmp1, x, x);
  fp_mulmod_montgomery(&tmp1, &tmp1, x);
  fp_add(&tmp1, &tmp1, &curve_b_montgomery);
  if (fp_legendre_sqrt_montgomery(y, &tmp1) == 1)
    return 1;
  else
    return 0;
}
...
void efp_set_random_montgomery(efp_t *P, gmp_randstate_t state) {
  ...
  while (1) {
    if (efp_compute_y_from_x_montgomery(&P->y, &P->x)) break;
