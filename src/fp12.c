...
int fp12_cmp(fp12_t *A, fp12_t *B) {
  if (!fp6_cmp(&A->x0, &B->x0) && !fp6_cmp(&A->x1, &B->x1))
    return 0;
  else
    return 1;
}
...
int fp12_cmp_ui(fp12_t *A, unsigned long int UI) {
  if (!fp6_cmp_ui(&A->x0, UI) && !fp6_cmp_ui(&A->x1, UI))
    return 0;
  else
    return 1;
}
...
int fp12_cmp_mpn(fp12_t *A, mp_limb_t *B) {
  if (!fp6_cmp_mpn(&A->x0, B) && !fp6_cmp_mpn(&A->x1, B))
    return 0;
  else
    return 1;
}
...
int fp12_cmp_zero(fp12_t *A) {
  if (!fp6_cmp_zero(&A->x0) && !fp6_cmp_zero(&A->x1))
    return 0;
  else
    return 1;
}
...
int fp12_cmp_one(fp12_t *A) {
  if (!fp6_cmp_one(&A->x0) && !fp6_cmp_zero(&A->x1))
    return 0;
  else
    return 1;
}
