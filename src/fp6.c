...
void fp6_mul_lazy_montgomery(fp6_t *ANS,fp6_t *A,fp6_t *B){
    ...
    fp2_mul_basis_nonmod_double(&tmp4_fpd2,&tmp6_fpd2);
    ...
    fp2_mul_basis_nonmod_double(&tmp4_fpd2,&tmp3_fpd2);
...
void fp6_mul_basis_nonmod_double(fpd6_t *ANS,fpd6_t *A){
...
void fp6_mul_basis_nonmod_single(fp6_t *ANS,fp6_t *A){
    static fp2_t tmp1_fp2,tmp2_fp2,tmp3_fp2;
    fp2_set(&tmp1_fp2,&A->x0);
    fp2_set(&tmp2_fp2,&A->x1);
    fp2_set(&tmp3_fp2,&A->x2);

    fp_sub_nonmod_single(&ANS->x0.x0,&tmp3_fp2.x0,&tmp3_fp2.x1);
    fp_add_nonmod_single(&ANS->x0.x1,&tmp3_fp2.x0,&tmp3_fp2.x1);
    fp_set(&ANS->x1.x0,&tmp1_fp2.x0);
    fp_set(&ANS->x1.x1,&tmp1_fp2.x1);
    fp_set(&ANS->x2.x0,&tmp2_fp2.x0);
    fp_set(&ANS->x2.x1,&tmp2_fp2.x1);
...
int  fp6_cmp(fp6_t *A,fp6_t *B){
    if(!fp2_cmp(&A->x0,&B->x0) && !fp2_cmp(&A->x1,&B->x1) && !fp2_cmp(&A->x2,&B->x2)) return 0;
    else return 1;
}

int  fp6_cmp_ui(fp6_t *A,unsigned long int UI){
    if(!fp2_cmp_ui(&A->x0,UI) && !fp2_cmp_ui(&A->x1,UI) && !fp2_cmp_ui(&A->x2,UI)) return 0;
    else return 1;
}

int  fp6_cmp_mpn(fp6_t *A,mp_limb_t *B){
    if(!fp2_cmp_mpn(&A->x0,B) && !fp2_cmp_mpn(&A->x1,B) && !fp2_cmp_mpn(&A->x2,B)) return 0;
    else return 1;
}

int  fp6_cmp_zero(fp6_t *A){
    if(!fp2_cmp_zero(&A->x0) && !fp2_cmp_zero(&A->x1) && !fp2_cmp_zero(&A->x2)) return 0;
    else return 1;
}

int  fp6_cmp_one(fp6_t *A){
    if(!fp2_cmp_one(&A->x0) && !fp2_cmp_zero(&A->x1) && !fp2_cmp_zero(&A->x2)) return 0;
    else return 1;
}
