...
int fp6_cmp(fp6_t *A, fp6_t *B) {
  if (!fp2_cmp(&A->x0, &B->x0) && !fp2_cmp(&A->x1, &B->x1) && !fp2_cmp(&A->x2, &B->x2))
    return 0;
  else
    return 1;
}
...
int fp6_cmp_ui(fp6_t *A, unsigned long int UI) {
  if (!fp2_cmp_ui(&A->x0, UI) && !fp2_cmp_ui(&A->x1, UI) && !fp2_cmp_ui(&A->x2, UI))
    return 0;
  else
    return 1;
}
...
int fp6_cmp_mpn(fp6_t *A, mp_limb_t *B) {
  if (!fp2_cmp_mpn(&A->x0, B) && !fp2_cmp_mpn(&A->x1, B) && !fp2_cmp_mpn(&A->x2, B))
    return 0;
  else
    return 1;
}
...
int fp6_cmp_zero(fp6_t *A) {
  if (!fp2_cmp_zero(&A->x0) && !fp2_cmp_zero(&A->x1) && !fp2_cmp_zero(&A->x2))
    return 0;
  else
    return 1;
}
...
int fp6_cmp_one(fp6_t *A) {
  if (!fp2_cmp_one(&A->x0) && !fp2_cmp_zero(&A->x1) && !fp2_cmp_zero(&A->x2))
    return 0;
  else
    return 1;
}
