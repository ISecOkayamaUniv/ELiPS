...
#define debug_get(msg) debug_getchar(__LINE__, msg)

void debug_getchar(int line, char *msg){
...
int w_naf_frt_mpn_size(int *dw,mp_limb_t *d,mp_size_t size,int w){
	...
	if(w==5){
        ...
        while(mpn_zero_p(tmp_d,size)!=1){
            if(tmp_d[0]%2==1){
                dw[i]=tmp_d[0]%n_int;
                if(dw[i]<=16) mpn_sub_ui(tmp_d,tmp_d,size,dw[i]);
...
void g1_test(int scm){
    ...
    printf("\n");
    printf("g1 test:");
    if(g1_cmp_efp12(&A_g1,&A_efp12)==0) printf("ok!\n");
    else printf("ng\n");
    //bench
    int i=0;
    float scm_time=0;
...
void g2_test(int scm){
    ...
    printf("\n");
    printf("g2 test:");
    if(g2_cmp_efp12(&A_g2,&A_efp12)==0) printf("ok!\n");
    else printf("ng\n");
