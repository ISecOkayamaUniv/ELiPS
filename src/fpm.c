...
void fpm_set_fp2cv(fpm_t *a, fp2cv_t *b) {
  int i, j;
  j = 0;
  for (i = 0; i < DEGREE_EXTENTION_FIELD_M; i++) {
    fp_set(&a->x[i], &b->x[j]);
    j++;
    if (i + 1 == DEGREE_EXTENTION_FIELD_2) {
      j = 0;
    }
  }
}

void fpm_set_fpm(fpm_t *a, fpm_t *b) {
  int i;
  for (i = 0; i < DEGREE_EXTENTION_FIELD_M; i++) {
    fp_set(&a->x[i], &b->x[i]);
  }
...
void fpm_mul(fpm_t *c, fpm_t *a, fpm_t *b) {
  ...
  static int x;
  int h = h_degree_m;
  static int m = DEGREE_EXTENTION_FIELD_M;
  static fp_t v[DEGREE_EXTENTION_FIELD_M + 1];
  ...
  if (x == 1) {
    fp_set(&tmp1, &v[m]);
    for (i = 2; i <= h; i++) {
      fp_add(&tmp1, &tmp1, &v[m]);
    }
    for (l = 0; l <= m - 1; l++) {
      fp_sub(&c->x[l], &tmp1, &v[l]);
    }
  }
  //h is even
  else {
...
void fpm_mul_fp(fpm_t *c, fpm_t *a, fp_t *b) {
  int i;
  for (i = 0; i < DEGREE_EXTENTION_FIELD_M; i++) {
    fp_mul(&c->x[i], &a->x[i], b);
  }
}
void fpm_mul_ui(fpm_t *c, fpm_t *a, unsigned long int b) {
  int i;
  for (i = 0; i < DEGREE_EXTENTION_FIELD_M; i++) {
    fp_mul_ui(&c->x[i], &a->x[i], b);
  }
}
...
void fpm_frobenius_times(fpm_t *b, fpm_t *a, unsigned long int number) {
  int i;
  fpm_t tmp;
  fpm_set(&tmp, a);
  for (i = 0; i < number; i++) {
    fpm_frobenius(&tmp, &tmp);
  }
  fpm_set(b, &tmp);
}
void fpm_inv(fpm_t *b, fpm_t *a) {
  ...
  fpm_frobenius(&conjugate, a);
  fpm_set(&product, &conjugate);
  for (i = 2; i < DEGREE_EXTENTION_FIELD_M; i++) {
    fpm_frobenius(&conjugate, &conjugate);
    fpm_mul(&product, &product, &conjugate);
  }
  fpm_mul(&norm, a, &product);
  fp_inv(&scalar, &norm.x[0]);
  fp_set_neg(&scalar, &scalar);
  fpm_mul_fp(b, &product, &scalar);
}
