...
void fpm2_set_fpm(fpm2_t *b, fpm_t *a) {
  int i;
  fpm_t tmp1;
  fpm_set_neg(&tmp1, a);
  for (i = 0; i < DEGREE_EXTENTION_FIELD_M2; i++) {
    fpm_set_fpm(&b->x[i], &tmp1);
  }
}
void fpm2_set_fp2cv(fpm2_t *b, fp2cv_t *a) {
  fpm_t tmp;
  fpm_set_fp2cv(&tmp, a);
  fpm2_set_fpm(b, &tmp);
}
...
void fpm2_mul(fpm2_t *c, fpm2_t *a, fpm2_t *b) {
  ...
  x = h % 2;
  if (x == 1) {
    fpm_mul_ui(&tmp1, &v[m], h);
    for (l = 0; l <= m - 1; l++) {
      fpm_sub(&c->x[l], &tmp1, &v[l]);
    }
  } else {
    for (l = 0; l <= m - 1; l++) {
      fpm_set_neg(&c->x[l], &v[l]);
    }
  }
}
...
void fpm2_frobenius_times(fpm2_t *b, fpm2_t *a, unsigned long int number) {
  int i;
  fpm2_t tmp;

  fpm2_set(&tmp, a);
  for (i = 0; i < number; i++) {
    fpm2_frobenius(&tmp, &tmp);
  }
  fpm2_set(b, &tmp);
}
...
void fpm2_cbrt(fpm2_t *b, fpm2_t *a) {
  fpm2_t n, c, c2, c3, r, h, g, d, tmp;
  unsigned long int s, i;
  mpz_t k, t, result, exp, aaa;
  //init
  mpz_init(t);
  mpz_init(k);
  mpz_init(exp);
  ...
  //step1
  mpz_pow_ui(t, prime_z, DEGREE_EXTENTION_FIELD_M * DEGREE_EXTENTION_FIELD_M2);  //p^m*m2-1
  mpz_sub_ui(t, t, 1);
  mpz_mod_ui(result, t, 3);
  s = 0;
  while (mpz_cmp_ui(result, 0) == 0) {
    mpz_tdiv_q_ui(t, t, 3);
    mpz_mod_ui(result, t, 3);
  ...
  fpm2_set_ui(&h, 1);  //h = 1
  mpz_set_ui(exp, 3);
  mpz_pow_ui(exp, exp, s - 1);  //exp = 3^(s-1)
  fpm2_pow(&g, &c, exp);        //g = c^3^(s-1)
  fpm2_inv(&c, &c);             //c = c^-1

  for (i = 1; i < s; i++) {
    mpz_tdiv_q_ui(exp, exp, 3);
    fpm2_pow(&d, &r, exp);
    fpm2_mul(&c2, &c, &c);   //c2 = c^2
    fpm2_mul(&c3, &c, &c2);  //c3 = c^3
    if (fpm2_cmp(&d, &g) == 0) {
      fpm2_mul(&h, &h, &c);   //h = h*c
      fpm2_mul(&r, &r, &c3);  //r = r*c^3
    } else if (fpm2_cmp_ui(&d, 1) != 0) {
      fpm2_mul(&h, &h, &c2);  //h = h*c^2
      fpm2_mul(&r, &r, &c3);
      fpm2_mul(&r, &r, &c3);  //r = r*(c^3)^2
    }
    fpm2_set(&c, &c3);  //c = c^3
  }
  //step4
