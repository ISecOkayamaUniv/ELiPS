...
void fp4cv_to_montgomery(fp4cv_t *b, fp4cv_t *a){
  fp_to_montgomery(&b->x0,&a->x0);
  fp_to_montgomery(&b->x1,&a->x1);
  fp_to_montgomery(&b->x2,&a->x2);
  fp_to_montgomery(&b->x3,&a->x3);
}
...
void fp4cv_frobenius_times(fp4cv_t *b, fp4cv_t *a, unsigned long int number){
  int i;
  fp4cv_t tmp;
  fp4cv_set(&tmp,a);
  for(i=0;i<number;i++){
    fp4cv_frobenius(&tmp,&tmp);
  }
  fp4cv_set(b,&tmp);
}
