#include "bench.h"
mpz_t legendre_power_z,sqrt_power_z;
mpz_t g1_power;
...
void power_init(){
    ...
    mpz_init(legendre_power_z);
    mpz_init(g1_power);

    //set legendre_power_z
    mpz_set(tmp,prime_z);
    mpz_sub_ui(legendre_power_z,tmp,1);
    mpz_div_ui(legendre_power_z,legendre_power_z,2);

    //set sqrt_power_z
    mpz_add_ui(sqrt_power_z,tmp,1);
    mpz_div_ui(sqrt_power_z,sqrt_power_z,4);

    //set g1_power
    mpz_tdiv_q(g1_power,efp_total,order_z);
}
void fp_sqrt_fast(fp_t *ANS,fp_t *A){
  fp_pow(ANS,A,sqrt_power_z);
}
